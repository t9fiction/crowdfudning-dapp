...
const CampaignDetails: React.FC = () => {
  ...
  const [campaign, setCampaign] = useState<any | null>(null); // Use `any` or the appropriate type
  const [isLoading, setIsLoading] = useState(false);
  const [amount, setAmount] = useState('');
  const [donators, setDonators] = useState<any[]>([]); // Use `any[]` or the appropriate type
  ...
  useEffect(() => {
    ...
    if (storedCampaign) {
      ...
      console.error('No campaign data found in localStorage');
      router.push('/'); // Redirect if no campaign data
    }
  }, [router]);

  // Fetch donators when the contract and address are ready
  useEffect(() => {
    if (contract && campaign) {
      const fetchDonators = async () => {
        try {
          const data = await getDonations(campaign.pId);
          console.log("Donators Data", campaign.pId)
          setDonators(data);
        } catch (error) {
  ...
  const handleDonate = async () => {
    ...
    try {
      ...
      router.push('/'); // Redirect after donation
    } catch (error) {
