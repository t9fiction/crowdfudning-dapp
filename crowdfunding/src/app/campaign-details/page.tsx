...
const CampaignDetails: React.FC<{ state: StateType }> = ({ state }) => {
  ...
  const getDonations = async (pId: string): Promise<Donation[]> => {
    try {
      const { data: _getDonators } = await useReadContract({
        address: CONTRACT_ADDRESS,
        abi: CONTRACT_ABI,
        functionName: 'getDonators',
        args: [pId],
          ...
          <div>
            ...
            <div className="mt-[20px] flex flex-row items-center flex-wrap gap-[14px]">
              <div className="w-[52px] h-[52px] flex items-center justify-center rounded-full bg-[#2c2f32] cursor-pointer">
                <img src={state.image} alt="user" className="w-[60%] h-[60%] object-contain" />
              </div>
              <div>
